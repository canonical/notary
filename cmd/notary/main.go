...
// Parses the command line arguments and returns the appropriate app context.
func parseCLIInput() (*config.NotaryAppContext, error) {
	...
	app, err := config.CreateAppContext(*configFilePtr)
	if err != nil {
		log.Fatalf("couldn't create app context: %s", err)
	}
	return app, nil
