...
func (db *Database) UpdateCertificateAuthority(filter CertificateAuthorityFilter, certPEM string, status string) error {
	ca, err := db.GetCertificateAuthority(filter)
	if err != nil {
		return err
	}
	if certPEM != "" {
		err = db.AddCertificateChainToCertificateRequest(ByCSRID(ca.CSRID), certPEM)
		if err != nil {
			return err
		}
		newCert, err := db.GetCertificate(ByCertificatePEM(certPEM))
		if err != nil {
			return err
		}
