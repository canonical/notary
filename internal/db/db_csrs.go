...
func (db *DatabaseRepository) ListCertificateRequests() ([]CertificateRequest, error) {
...
func (db *DatabaseRepository) ListCertificateRequestsWithoutCAS() ([]CertificateRequest, error) {
...
func (db *DatabaseRepository) ListCertificateRequestWithCertificates() ([]CertificateRequestWithChain, error) {
...
func (db *DatabaseRepository) ListCertificateRequestWithCertificatesWithoutCAS(filter *CSRFilter) ([]CertificateRequestWithChain, error) {
	if filter == nil || filter.OwnerUserID == nil {
...
func (db *DatabaseRepository) GetCertificateRequest(filter CSRFilter) (*CertificateRequest, error) {
...
func (db *DatabaseRepository) GetCertificateRequestAndChain(filter CSRFilter) (*CertificateRequestWithChain, error) {
	csrRow := filter.AsCertificateRequestWithChain()
...
func (db *DatabaseRepository) CreateCertificateRequest(csr string, userID int64) (int64, error) {
	if err := ValidateCertificateRequest(csr); err != nil {
		return 0, err
	}
	row := CertificateRequest{CSR: csr}
	if userID > 0 {
		row.UserID = &userID
	}
...
func (db *DatabaseRepository) RejectCertificateRequest(filter CSRFilter) error {
...
func (db *DatabaseRepository) DeleteCertificateRequest(filter CSRFilter) error {
	csrRow := filter.AsCertificateRequest()
