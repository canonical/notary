...
func ListCertificateAuthorities(env *HandlerConfig) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		caResponse := make([]db.CertificateAuthorityDenormalized, len(cas))
		for i, ca := range cas {
			caResponse[i] = db.CertificateAuthorityDenormalized{
...
func CreateCertificateAuthority(env *HandlerConfig) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		certPEM, privPEM, csrPEM := createCertificateAuthority(params)
		if certPEM == "" || privPEM == "" {
			writeError(w, http.StatusInternalServerError, "Failed to create certificate authority")
			return
		}
		err := env.DB.CreateCertificateAuthority(csrPEM, privPEM, certPEM)
		if err != nil {
			writeError(w, http.StatusInternalServerError, "Failed to create certificate authority")
			return
		}
...
func PostCertificateAuthorityCertificate(env *HandlerConfig) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		err = env.DB.UpdateCertificateAuthority(db.ByCertificateAuthorityID(idNum), createCertificateParams.CertificateChain, "Active")
		if err != nil {
			log.Println(err)
			if errors.Is(err, sqlair.ErrNoRows) {
				writeError(w, http.StatusNotFound, "Not Found")
