...
import { RequiredCAParams, deleteCA, makeCALegacy, revokeCA } from "@/queries"
...
export function CertificateAuthoritiesTable({ cas: rows, setAsideOpen }: TableProps) {
  ...
  const deleteMutation = useMutation({
    mutationFn: deleteCA,
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['cas'] }),
  });
  ...
  const makeLegacyMutation = useMutation({
    mutationFn: makeCALegacy,
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['cas'] }),
  })

  const mutationFunc = (mutation: UseMutationResult<any, unknown, RequiredCAParams, unknown>, params: RequiredCAParams) => {
  ...
  const handleDelete = (id: number) => {
    setConfirmationModalData({
      onMouseDownFunc: () => mutationFunc(deleteMutation, { id: id.toString(), authToken: auth.user?.authToken || "" }),
  ...
  const handleRevoke = (id: number) => {
    setConfirmationModalData({
      onMouseDownFunc: () => mutationFunc(revokeMutation, { id: id.toString(), authToken: auth.user?.authToken || "" }),
      warningText: "Revoking a Certificate will remove the ability for this CA to sign certificates. This action cannot be undone.",
    });
  };

  const handleMakeLegacy = (id: number) => {
    setConfirmationModalData({
      onMouseDownFunc: () => mutationFunc(makeLegacyMutation, { id: id.toString(), authToken: auth.user?.authToken || "" }),
      warningText: "Making a Certificate Legacy will convert it from the table. This action cannot be undone.",
    });
  }
  ...
  console.log(auth.user?.activeCA)
  ...
  const carows = rows.map((caEntry) => {
    ...
    const isSelfSigned = certObj && certObj?.commonName === certObj?.issuerCommonName;
