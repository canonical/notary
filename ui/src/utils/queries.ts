...
export async function postCertToID(params: RequiredCSRParams) {
	...
	return fetchAPI(`/api/v1/certificate_requests/${params.id}/certificate`, {
		method: "post",
		headers: {
...
export async function changePassword(changePasswordForm: {
	...
	return fetchAPI(`/api/v1/accounts/${changePasswordForm.id}/change_password`, {
		method: "POST",
		body: JSON.stringify({ password: changePasswordForm.password }),
	});
}

export async function ListUsers(_params: {}): Promise<UserEntry[]> {
...
export async function postCertToCA(
	...
	return fetchAPI(`/api/v1/certificate_authorities/${params.id}/certificate`, {
		method: "post",
		headers: {
			"Content-Type": "application/json",
		},
